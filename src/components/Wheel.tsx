...
import { easeOutCirc, easeOutElastic } from 'easing-utils'
...
const itemConfig = { imageRadius: 0.525, imageScale: 0.525, imageRotation: 0, backgroundColor: 'transparent' }

let wheelInstance: any
let lockWheel: boolean | undefined

const loadWheel = async ({ current }: { current: HTMLElement | null }, movies: Movie[]) => {
  const items = (await Promise.all(movies.map(fetchMovies)))?.map?.(toWheelItems)
...
const toWheelItems = ({ Search: [{ Poster, imdbID: label }] }: { Search: [Movie] }) => ({
  label,
  image: createImageElement(Poster),
  ...itemConfig,
})

const toDecodeImage = async ({ image }: { image: HTMLImageElement }): Promise<void> => await image.decode()
...
const toTitleCase = ({ Title, Year }: Movie) => ({
  Title: Title.replace(/\w\S*/g, (text: string) => text.charAt(0).toUpperCase() + text.substring(1).toLowerCase()),
  Year,
})

const fetchMovieData = async ({ Title, Year }: Movie) =>
  (await fetch(`http://www.omdbapi.com/?apikey=44648a33&s=${Title}&y=${Year}&type=movie`)).json()

const fetchMovies = pipe(toTitleCase, fetchMovieData)

const initWheel = async (wheelContainer: React.MutableRefObject<null>, movies: Movie[]) => {
...
const centerWheelItem = ({ currentIndex, type }: { currentIndex: number; type?: string }) => {
  let spinProps
  switch (type) {
    case 'elastic':
      spinProps = [currentIndex, 2000, true, 0, getDirection(currentIndex), easeOutElastic]
      break
    default:
...
const getDirection = (currentIndex: number) => {
  ...
  const wheelRelativeRotation = rotation < 0 ? wheelAbsoluteRotation : 360 - wheelAbsoluteRotation
